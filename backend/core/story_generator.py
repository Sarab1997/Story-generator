...
class StoryGenerator:
    ...
    def _get_llm(cls):
        ...
        direct_api_key = os.getenv("OPENAI_API_KEY")
        if direct_api_key:
            return ChatOpenAI(model="gpt-4o-mini", api_key=direct_api_key)
        
        # If no API key found, raise error
        raise ValueError("No OpenAI API key found. Please set OPENAI_API_KEY in .env file")

    @classmethod
    def generate_story(cls, session_id: str, theme: str = "fantasy") -> Story:
        """
        Generate a story - creates and manages its own database session.
        Returns the story object after committing to database.
        
        Args:
            session_id: User session identifier
            theme: Story theme (default: "fantasy")
            
        Returns:
            Story: The created story with all nodes
        """
        db = SessionLocal()
        try:
            llm = cls._get_llm()
